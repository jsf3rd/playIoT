...
type
  ...
  // en:Acquisition mode
type
  MV_CAM_ACQUISITION_MODE = (MV_ACQ_MODE_SINGLE = 0, // ch:单帧模式 | en:Single Mode
    MV_ACQ_MODE_MUTLI = 1, // ch:多帧模式 | en:Multi Mode
    MV_ACQ_MODE_CONTINUOUS = 1); // ch:持续采集模式 | en:Continuous Mode

  // ch:增益模式 | en:Gain Mode
type
  MV_CAM_GAIN_MODE = (MV_GAIN_MODE_OFF = 0, // ch:关闭 | en:Single Mode
    MV_GAIN_MODE_ONCE = 1, // ch:一次 | en:Multi Mode
    MV_GAIN_MODE_CONTINUOUS = 2); // ch:连续 | en:Continuous Mode

  // ch:曝光模式 | en:Exposure Mode
type
  MV_CAM_EXPOSURE_MODE = (MV_EXPOSURE_MODE_TIMED = 0, MV_EXPOSURE_MODE_TRIGGER_WIDTH = 1);

  // ch:自动曝光模式 | en:Auto Exposure Mode
type
  MV_CAM_EXPOSURE_AUTO_MODE = (MV_EXPOSURE_AUTO_MODE_OFF = 0, // ch:关闭 | en:Off
    MV_EXPOSURE_AUTO_MODE_ONCE = 1, // ch:一次 | en:Once
    MV_EXPOSURE_AUTO_MODE_CONTINUOUS = 2); // ch:连续 | en:Continuous

type
  MV_CAM_TRIGGER_MODE = (MV_TRIGGER_MODE_OFF = 0, // ch:关闭 | en:Off
    MV_TRIGGER_MODE_ON = 1); // ch:打开 | en:ON

type
  MV_CAM_GAMMA_SELECTOR = (MV_GAMMA_SELECTOR_USER = 0, MV_GAMMA_SELECTOR_SRGB = 1);

type
  MV_CAM_BALANCEWHITE_AUTO = (MV_BALANCEWHITE_AUTO_OFF = 0, MV_BALANCEWHITE_AUTO_CONTINUOUS = 1,
    MV_BALANCEWHITE_AUTO_ONCE = 2);

type
  MV_CAM_TRIGGER_SOURCE = (MV_TRIGGER_SOURCE_LINE0 = 0, MV_TRIGGER_SOURCE_LINE1 = 1,
    MV_TRIGGER_SOURCE_LINE2 = 2, MV_TRIGGER_SOURCE_LINE3 = 3, MV_TRIGGER_SOURCE_COUNTER0 = 4,
    MV_TRIGGER_SOURCE_RESERVED = 5, MV_TRIGGER_SOURCE_RESERVED2 = 6, MV_TRIGGER_SOURCE_SOFTWARE = 7,
    MV_TRIGGER_SOURCE_FrequencyConverter = 8);

type
  MV_GIGE_TRANSMISSION_TYPE = (MV_GIGE_TRANSTYPE_UNICAST = $0, // ch:表示单播(默认) | en:Unicast mode
    ...
    // ch:表示设置了单播，但本实例不接收图像数据 | en:Unicast without receive data
    MV_GIGE_TRANSTYPE_MULTICAST_WITHOUT_RECV = $00010001);
